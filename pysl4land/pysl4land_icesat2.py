...
def get_beam_lst(input_file, strong_only, weak_only):
    ...
    if strong_only == True:
        ...
        icesat2_beams = [
            "gt1" + strongOrientDict[orientation],
            "gt2" + strongOrientDict[orientation],
        ...
        icesat2_beams = [
            "gt1" + weakOrientDict[orientation],
            "gt2" + weakOrientDict[orientation],
...
def get_segment_polygons(latitude, longitude, along_size=100.0, across_size=13.0):
    ...
    utm_zone = pysl4land.pysl4land_utils.latlon_to_mode_utm_zone_number(
        latitude, longitude
    )
    ...
    icesat2_pts_wgs84_gdf = geopandas.GeoDataFrame(
        icesat2_pts_df,
        crs="EPSG:4326",
        geometry=geopandas.points_from_xy(
            icesat2_pts_df.longitude, icesat2_pts_df.latitude
        ),
    )
    ...
    logger.debug(
        "along_size_h: {} \t\t across_size_h: {}".format(along_size_h, across_size_h)
    )
    ...
    icesat2_pts_utm_gdf["ul_x_rot"] = (
        icesat2_pts_utm_gdf.geometry.x
        + theta_cos * (icesat2_pts_utm_gdf["ul_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_sin * (icesat2_pts_utm_gdf["ul_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    icesat2_pts_utm_gdf["ul_y_rot"] = (
        icesat2_pts_utm_gdf.geometry.y
        + -theta_sin * (icesat2_pts_utm_gdf["ul_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_cos * (icesat2_pts_utm_gdf["ul_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    ...
    icesat2_pts_utm_gdf["ur_x_rot"] = (
        icesat2_pts_utm_gdf.geometry.x
        + theta_cos * (icesat2_pts_utm_gdf["ur_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_sin * (icesat2_pts_utm_gdf["ur_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    icesat2_pts_utm_gdf["ur_y_rot"] = (
        icesat2_pts_utm_gdf.geometry.y
        + -theta_sin * (icesat2_pts_utm_gdf["ur_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_cos * (icesat2_pts_utm_gdf["ur_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    ...
    icesat2_pts_utm_gdf["lr_x_rot"] = (
        icesat2_pts_utm_gdf.geometry.x
        + theta_cos * (icesat2_pts_utm_gdf["lr_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_sin * (icesat2_pts_utm_gdf["lr_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    icesat2_pts_utm_gdf["lr_y_rot"] = (
        icesat2_pts_utm_gdf.geometry.y
        + -theta_sin * (icesat2_pts_utm_gdf["lr_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_cos * (icesat2_pts_utm_gdf["lr_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    ...
    icesat2_pts_utm_gdf["ll_x_rot"] = (
        icesat2_pts_utm_gdf.geometry.x
        + theta_cos * (icesat2_pts_utm_gdf["ll_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_sin * (icesat2_pts_utm_gdf["ll_y"] - icesat2_pts_utm_gdf.geometry.y)
    )
    icesat2_pts_utm_gdf["ll_y_rot"] = (
        icesat2_pts_utm_gdf.geometry.y
        + -theta_sin * (icesat2_pts_utm_gdf["ll_x"] - icesat2_pts_utm_gdf.geometry.x)
        + theta_cos * (icesat2_pts_utm_gdf["ll_y"] - icesat2_pts_utm_gdf.geometry.y)
    )

    def _polygonise_2Dcells(df_row):
        return Polygon(
            [
                (df_row.ul_x_rot, df_row.ul_y_rot),
                (df_row.ur_x_rot, df_row.ur_y_rot),
                (df_row.lr_x_rot, df_row.lr_y_rot),
                (df_row.ll_x_rot, df_row.ll_y_rot),
            ]
        )
    ...
    icesat2_polys_utm_gdf = geopandas.GeoDataFrame(
        icesat2_pts_df, crs="EPSG:{}".format(epsg_code), geometry=polys
    )
    logger.debug("Creating geopandas polygons WGS84 dataframe")
    icesat2_polys_wgs84_gdf = icesat2_polys_utm_gdf.to_crs(
        "EPSG:4326".format(epsg_code)
    )

    return numpy.asarray(icesat2_polys_wgs84_gdf.geometry.values)


def get_icesat2_alt08_beam_as_gdf(
    input_file,
    icesat2_beam_name,
    use_seg_polys=False,
    out_epsg_code=4326,
    ...
    icesat2_atlas_sdp_gps_epoch = icesat2_h5_file["ancillary_data"][
        "atlas_sdp_gps_epoch"
    ][0]

    gps_epoch_delta_time = icesat2_land_beam["delta_time"] + icesat2_atlas_sdp_gps_epoch
    utc_delta_time = [
        datetime(1980, 1, 6) + timedelta(seconds=x - (37 - 19))
        for x in gps_epoch_delta_time
    ]
    ...
    icesat2_beam_df = pandas.DataFrame(
        {
            ...
            "canopy_h_metrics_abs_rh25": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 0
            ],
            "canopy_h_metrics_abs_rh50": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 1
            ],
            "canopy_h_metrics_abs_rh60": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 2
            ],
            "canopy_h_metrics_abs_rh70": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 3
            ],
            "canopy_h_metrics_abs_rh75": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 4
            ],
            "canopy_h_metrics_abs_rh80": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 5
            ],
            "canopy_h_metrics_abs_rh85": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 6
            ],
            "canopy_h_metrics_abs_rh90": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 7
            ],
            "canopy_h_metrics_abs_rh95": icesat2_beam_canopy["canopy_h_metrics_abs"][
                :, 8
            ],
    ...
    if use_seg_polys:
        ...
        polys = get_segment_polygons(
            latitude_arr, longitude_arr, along_size=100.0, across_size=13.0
        )
        icesat2_beam_gdf = geopandas.GeoDataFrame(
            icesat2_beam_df, crs="EPSG:4326", geometry=polys
        )
    else:
        icesat2_beam_gdf = geopandas.GeoDataFrame(
            icesat2_beam_df,
            crs="EPSG:4326",
            geometry=geopandas.points_from_xy(
                icesat2_beam_df.longitude, icesat2_beam_df.latitude
            ),
        )
...
def icesat2_alt08_beams_gpkg(
    ...
    for icesat2_beam_name in icesat2_beams:
        ...
        icesat2_beam_gdf = get_icesat2_alt08_beam_as_gdf(
            input_file, icesat2_beam_name, use_seg_polys, out_epsg_code
        )
